...
export class ForgotPasswordController {
  ...
  async sendResetLinkEmail(@Body() data: SendResetLinkParams): Promise<any> {
    const { email } = data;
    const user = await this.userService.firstOrFail({ where: { email: this.userService.sanitizeEmail(email) } });
  ...
  async reset(@Body() data: ResetPasswordParams): Promise<any> {
    ...
    const password_reset = await this.passwordResetService.firstOrFail({ where: { token } });
    if (this.passwordResetService.isExpired(password_reset)) {
      throw new BadRequestException('Token is expired');
    }
    await this.passwordResetService.expire(token);
    const user = await this.userService.first({ where: { email: password_reset.email } });
